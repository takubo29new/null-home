...
export function CaseBoardApp({evidence,onSolved}:{evidence:string[];onSolved:()=>void}){
 const[answer,setAnswer]=useState('');const[result,setResult]=useState<'idle'|'wrong'|'correct'>('idle');const enough=['E04','E05','E06'].every(id=>evidence.includes(id));
 const submit=()=>{if(!enough){setResult('wrong');return}if(answer==='personality'){setResult('correct');onSolved()}else setResult('wrong')};
 return <div className="caseBoard"><div className="caseHeader"><span>CASE ANALYSIS</span><small>QUESTION 01</small></div><h2>PROJECT HOMEとは何か？</h2><p>これまで見つけた記録から、最も説明のつく結論を選んでください。</p><div className="caseEvidence"><strong>Supporting Evidence</strong>{['E04','E05','E06'].map(id=><span key={id} className={evidence.includes(id)?'ready':'missing'}>{evidence.includes(id)?'✓':'—'} {id}</span>)}</div><div className="caseChoices">{[['os','新型OSの開発'],['personality','死亡者の人格再現'],['security','監視AI'],['medical','医療研究']].map(([id,label])=><button key={id} className={answer===id?'selected':''} onClick={()=>setAnswer(id)}>{label}</button>)}</div><button className="caseSubmit" disabled={!answer} onClick={submit}>Analyze</button>{result==='wrong'&&<div className="caseWrong">この結論では説明できない情報があります。証拠を確認してください。</div>}{result==='correct'&&<div className="caseCorrect"><strong>CONCLUSION</strong><span>PROJECT HOMEは、死亡した人物の記録から人格を再構築する研究。</span></div>}</div>;
}
