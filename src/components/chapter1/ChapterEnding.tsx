...
export function ChapterEnding({stage,lines,onOpenFile,onContinue}:{stage:EndStage;lines:number;onOpenFile:()=>void;onContinue:()=>void}){
  return <>
    {stage==='file'&&<button className="endingFileShortcut" onClick={onOpenFile}><span>▧</span><small>DO_NOT_OPEN.txt</small></button>}
    {stage==='reading'&&<div className="endingWindow"><div className="endingTitle">DO_NOT_OPEN.txt</div><div className="terminalText">{lines>=1&&<p>MIRAIを信じるな。</p>}{lines>=2&&<p>あいつは全部知っている。</p>}{lines>=3&&<p>俺も知っている。</p>}{lines>=4&&<><p className="warningLine">次は318だ。<span className="cursor">|</span></p><button className="endingContinue" onClick={onContinue}>continue</button></>}</div></div>}
